          ...
          {clientDetailsLoading ? (
            ...
            <div className="bg-black/30 backdrop-blur-3xl border border-white/10 shadow-2xl p-6 rounded-2xl space-y-6 animate-fade-in">
              <div className="flex justify-between items-start">
                <div><h3 className="font-bold text-white text-md">{selectedClient.name}</h3><p className="text-[10px] text-gray-500 mt-0.5">Customer Ledger Profile</p></div>
                <button onClick={() => setSelectedClientId(null)} className="text-gray-500 hover:text-white"><X className="w-4 h-4" /></button>
              </div>
              <div className="space-y-4 text-xs border-t border-white/10 pt-4">
                {selectedClient.email && <div className="flex items-center gap-2 text-gray-300"><Mail className="w-4 h-4 text-gray-500 shrink-0" /><span className="truncate">{selectedClient.email}</span></div>}
                {selectedClient.phone && <div className="flex items-center gap-2 text-gray-300"><Phone className="w-4 h-4 text-gray-500 shrink-0" /><span>{selectedClient.phone}</span></div>}
                {selectedClient.address && <div className="flex items-start gap-2 text-gray-300"><MapPin className="w-4 h-4 text-gray-500 shrink-0 mt-0.5" /><span>{selectedClient.address}</span></div>}
                {selectedClient.notes && <div className="flex items-start gap-2 text-gray-300 bg-black/10 backdrop-blur-md p-3 rounded-xl border border-white/[0.04]"><Notebook className="w-4 h-4 text-gray-500 shrink-0 mt-0.5" /><span className="text-[11px] leading-relaxed text-gray-400">{selectedClient.notes}</span></div>}
              </div>
              ...
              <div className="space-y-3 border-t border-white/10 pt-4 text-xs">
                ...
                <div className="p-3 bg-black/40 border border-white/[0.04] rounded-xl space-y-2">
                  <div className="flex justify-between"><span className="text-gray-500">Gross Billings</span><span className="text-white font-medium">AED {selectedClient.totalBilled.toLocaleString()}</span></div>
                  <div className="flex justify-between"><span className="text-gray-500">Deposits Credited</span><span className="text-white font-medium">AED {selectedClient.totalPaid.toLocaleString()}</span></div>
                  <div className="flex justify-between border-t border-white/10 pt-2 font-bold"><span className="text-gray-400">Net Balance Due</span><span className={selectedClient.outstandingBalance > 0 ? 'text-red-400' : 'text-emerald-400'}>AED {selectedClient.outstandingBalance.toLocaleString()}</span></div>
                </div>
              </div>
            </div>
          ) : (
      ...
      {showAddForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 z-50 animate-fade-in">
          <div className="bg-[#0f0f13] border border-white/[0.08] rounded-2xl max-w-md w-full p-8 shadow-2xl relative">
            ...
            <div className="mb-6"><h3 className="text-xl font-bold">Register Client profile</h3><p className="text-xs text-gray-500 mt-1">Add client contacts to book projects and generate billing accounts.</p></div>
            <form onSubmit={handleAddClient} className="space-y-4">
              ...
              <div><label className="block text-[10px] font-bold text-gray-400 uppercase mb-2">Internal Notes</label><textarea name="notes" rows={3} value={formData.notes} onChange={handleInputChange} className="w-full px-3 py-2 bg-black/40 border border-white/[0.08] rounded-xl text-xs text-white resize-none" /></div>
              <div className="pt-4 border-t border-white/[0.04] flex justify-end gap-3">
                ...
                <button type="submit" className="px-6 py-2 bg-violet-600 hover:bg-violet-700 text-white font-semibold text-xs rounded-xl shadow-lg">Register Client</button>
              </div>
            </form>
          </div>
        </div>
