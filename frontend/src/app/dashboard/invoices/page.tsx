...
export default function InvoicesPage() {
  ...
  const handleRecordPayment = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const next = await recordPayment({ invoiceId: selectedInv.id, body: { amount: Number(paymentAmount), paymentMethod, referenceNumber: refNum || undefined } }).unwrap();
      setSelectedInv(next);
              ...
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody>
                  {invoices.map(inv => {
                    ...
                    return (
                      <tr key={inv.id} onClick={() => { setSelectedInv(inv); setShowPayForm(false); }}
                        className={`border-b border-white/[0.03] hover:bg-black/10 backdrop-blur-md cursor-pointer transition-colors ${selectedInv?.id === inv.id ? 'bg-black/20 backdrop-blur-xl' : ''}`}>
                        <td className="p-4 font-mono font-bold text-violet-400">{inv.invoiceNumber}</td>
                        <td className="p-4"><p className="font-semibold text-white">{inv.client?.name}</p><span className="text-[10px] text-gray-500 block mt-0.5">{inv.event?.title || 'Standalone Campaign'}</span></td>
                        <td className="p-4 font-mono text-gray-400">{new Date(inv.dueDate).toISOString().split('T')[0]}</td>
                        <td className="p-4"><span className={`px-2 py-0.5 rounded-md text-[9px] font-bold ${inv.status === 'PAID' ? 'bg-emerald-500/10 text-emerald-400' : inv.status === 'PARTIALLY_PAID' ? 'bg-amber-500/10 text-amber-400' : 'bg-red-500/10 text-red-400'}`}>{inv.status}</span></td>
                        <td className="p-4 font-bold text-white text-right">AED {Number(inv.totalAmount).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="bg-black/30 backdrop-blur-3xl border border-white/10 shadow-2xl p-6 rounded-2xl space-y-6 animate-fade-in text-xs relative">
              ...
              <div className="bg-[#0f0f13] border border-white/[0.06] rounded-xl p-5 space-y-4 shadow-inner relative">
                ...
                <div className="flex justify-between items-start border-b border-white/[0.03] pb-3">
                  <div>
                    {selectedInv.company?.logoUrl ? <img src={selectedInv.company.logoUrl} alt="Logo" className="w-10 h-10 object-cover rounded-lg border border-white/10 mb-2" /> : <div className="w-8 h-8 rounded-lg bg-violet-600 flex items-center justify-center font-bold text-white mb-2">{selectedInv.company?.name[0]}</div>}
                ...
                <div><p className="text-[9px] text-gray-500 uppercase">Debtor Info:</p><p className="font-bold text-white mt-1">{selectedInv.client?.name}</p><p className="text-gray-400 mt-0.5">{selectedInv.client?.email}</p></div>
                <div className="border-t border-white/[0.03] pt-3 space-y-2 text-gray-400">
                  <div className="flex justify-between"><span>Campaign Subtotal</span><span className="text-white">AED {Number(selectedInv.subtotal).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span></div>
                  <div className="flex justify-between"><span>Tax VAT ({selectedInv.taxRate}%)</span><span className="text-white">AED {Number(selectedInv.taxAmount).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span></div>
                  {Number(selectedInv.discount) > 0 && <div className="flex justify-between text-red-400"><span>Discount</span><span>- AED {Number(selectedInv.discount).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span></div>}
                  <div className="flex justify-between border-t border-white/10 pt-2 font-bold text-[11px] text-white"><span>Grand Invoiced Total</span><span>AED {Number(selectedInv.totalAmount).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span></div>
                  <div className="flex justify-between text-amber-400 border-t border-white/[0.03] pt-2 font-bold"><span>Outstanding Balance Due</span><span>AED {getInvoiceBalance(selectedInv).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span></div>
                </div>
                {selectedInv.qrCodeData && (
                  <div className="border-t border-white/[0.04] pt-3 flex gap-3 items-center bg-black/40 p-2.5 rounded-xl border border-white/[0.03]">
                    ...
                    <div className="overflow-hidden"><p className="text-[8px] font-bold text-gray-500 uppercase tracking-widest">Compliance QR Verification</p><p className="text-[8px] font-mono text-gray-400 truncate mt-0.5">{selectedInv.qrCodeData}</p></div>
                  </div>
                )}
              </div>
                  ...
                  <div className="space-y-2 max-h-32 overflow-y-auto">
                    {selectedInv.payments.map((p: any) => (
                      <div key={p.id} className="flex justify-between items-center bg-black/20 p-2.5 border border-white/[0.03] rounded-lg">
                        <div><p className="font-semibold text-white">AED {Number(p.amount).toLocaleString()}</p><span className="text-[9px] text-gray-500">{p.paymentMethod} &bull; Ref: {p.referenceNumber || 'N/A'}</span></div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 z-50 animate-fade-in">
          <div className="bg-[#0f0f13] border border-white/[0.08] rounded-2xl max-w-md w-full p-8 shadow-2xl relative">
            ...
            <form onSubmit={handleCreateInvoice} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-[10px] font-bold text-gray-400 uppercase mb-2">Tax VAT (%)</label><input type="number" required min={0} value={taxRate} onChange={e => setTaxRate(Number(e.target.value))} className="w-full px-3 py-2 bg-black/40 border border-white/[0.08] rounded-xl text-xs text-white" /></div>
                <div><label className="block text-[10px] font-bold text-gray-400 uppercase mb-2">Due Date</label><input type="date" required value={dueDate} onChange={e => setDueDate(e.target.value)} className="w-full px-3 py-2 bg-black/40 border border-white/[0.08] rounded-xl text-xs text-gray-400 focus:outline-none" /></div>
              </div>
              <div className="bg-black/40 p-4 border border-white/[0.04] rounded-xl space-y-2 text-xs text-gray-400">
                <div className="flex justify-between"><span>Subtotal</span><span className="text-white">AED {subtotal.toLocaleString()}</span></div>
                <div className="flex justify-between"><span>VAT ({taxRate}%)</span><span className="text-white">AED {calcTaxAmount().toLocaleString()}</span></div>
                {discount > 0 && <div className="flex justify-between text-red-400"><span>Discount</span><span>- AED {discount.toLocaleString()}</span></div>}
                <div className="flex justify-between border-t border-white/[0.06] pt-2 font-bold text-white"><span>Grand Total</span><span>AED {calcTotalAmount().toLocaleString()}</span></div>
              </div>
              <div className="pt-4 border-t border-white/[0.04] flex justify-end gap-3">
                ...
                <button type="submit" className="px-6 py-2 bg-violet-600 hover:bg-violet-700 text-white font-semibold text-xs rounded-xl shadow-lg">Generate Invoice</button>
              </div>
            </form>
          </div>
        </div>
