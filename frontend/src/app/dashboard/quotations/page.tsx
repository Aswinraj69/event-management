...
export default function QuotationsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const qRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://event-management-production-b372.up.railway.app'}/api/quotations`, {
  ...
  const handleCreateQuo = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://event-management-production-b372.up.railway.app'}/api/quotations`, {
        method: 'POST',
        headers: {
  ...
  const handleUpdateStatus = async (id: string, status: string) => {
    const token = localStorage.getItem('evento_token');
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://event-management-production-b372.up.railway.app'}/api/quotations/${id}/status`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
